...
if (($_POST['username']) != "" && ($_POST['password']) != "" && ($_POST['firstname']) != "" && ($_POST['lastname']) != "" && ($_POST['email']) != "" && ($_POST['housenumber']) != "" && ($_POST['city']) != "" && ($_POST['country']) != "") {
	...
	$user_role = 2;
	$sql = "INSERT INTO users(user_salutation, user_nickname, user_password, user_firstname, user_lastname, user_email, user_adress, user_housenumber, user_city, user_postcode, user_country, user_role) VALUES ('$user_salutation', '$user_nickname', '$user_password', '$user_firstname', '$user_lastname', '$user_email', '$user_adress', '$user_housenumber', '$user_city', '$user_postcode', '$user_country', '$user_role')";
	mysql_query($sql) or die ("Noob");
